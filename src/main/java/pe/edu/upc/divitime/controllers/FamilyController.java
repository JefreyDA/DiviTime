...
@RestController
    ...
    public class FamilyController {
        ...
        public ResponseEntity<List<FamilyDTO>> listar() {
            ModelMapper m = new ModelMapper();

            List<FamilyDTO> listaFamilias = fS.list().stream()
                    .map(y -> m.map(y, FamilyDTO.class))
                    .collect(Collectors.toList());

            return ResponseEntity.ok(listaFamilias);
        }

        @PostMapping("/insertar")
        public ResponseEntity<FamilyGeneralDTO> insertar(@RequestBody FamilyGeneralDTO dto) {
            ModelMapper m = new ModelMapper();

            Family f = m.map(dto, Family.class);

            User u = new User();
            u.setIdusuario(dto.getIdCreatorFamily());
            f.setCreatorFamily(u);

            Family family = fS.insert(f);

            FamilyGeneralDTO responseDTO = m.map(family, FamilyGeneralDTO.class);
            responseDTO.setIdCreatorFamily(family.getCreatorFamily().getIdusuario());
        ...
        public ResponseEntity<?> BuscarId(@PathVariable int id) {
            ...
            if (family.isPresent()) {
                ...
                dto.setIdCreatorFamily(family.get().getCreatorFamily().getIdusuario());
    ...
    @GetMapping("/familias-por-fechas")
    public ResponseEntity<?> obtenerFamiliasPorRangoFechas(
